...
In addition, in regards to each of the sections of the code we implemented, each of the section serves to create one whole logistic regression model that gives predictions and tests the results. More specifically, in the model, Parameters are initialized with 0's with dimensions being an input value and 1. Gradient descent minimizes cost. Forward/backward propagation first moves the data from input layer to output layer and then moves it from output layer to input layer. Sigmoid function transforms the input data into a value between 0 and 1 and computes the likelihood of the observed data given the model's parameters. Furthermore, Gradient descent minimizes errors between predicted and actual results. In forward propagation, data is passed through the network's layers to generate a prediction or output. Backward propagation is an elegant method to calculate how changes to any of the weights or biases of the neural network will affect the accuracy of model predictions.
...
\LIST{
\item Wrote the Experimental Results and Analysis section
\item Created pie chart of Titanic survival rate and the bar chart of the iteration counts
}
